...
class RhobanDataset(Dataset):
    def __init__(self,data_root, split='train', num_point=4096, block_size=1.0, sample_rate=1.0, transform=None):
        ...
        for file in tqdm(files):
            ...
            room_data = np.load(file_path)  # xyzrgbl, N*7
            points, labels = room_data[:, 0:6], room_data[:, 6]  # xyzrgb, N*6; l, N
    ...
    def __getitem__(self, idx):
        ...
        selected_points = points[selected_point_idxs, :]  # num_point * 6
        current_points = np.zeros((self.num_point, 9))  # num_point * 9
        current_points[:, 6] = selected_points[:, 0] / self.room_coord_max[idx][0]
        current_points[:, 7] = selected_points[:, 1] / self.room_coord_max[idx][1]
        current_points[:, 8] = selected_points[:, 2] / self.room_coord_max[idx][2]
        selected_points[:, 0] = selected_points[:, 0] - center[0]
        selected_points[:, 1] = selected_points[:, 1] - center[1]
        selected_points[:, 3:6] /= 255.0
        current_points[:, 0:6] = selected_points
